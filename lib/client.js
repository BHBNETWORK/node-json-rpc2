        ...
        response.on('end', function(){
           var error, result;
            ...
            if(response.statusCode=== 400){
                ...
                error = new Error('Connection Rejected : 401 Unauthorized - '+data);
            } 
            else if(response.statusCode === 403){
                error = new Error('Connection Rejected : 403 Forbidden - '+data);
            }
            else if(response.statusCode === 500){
                error = new Error (JSON.stringify ({statusCode:response.statusCode, data:JSON.parse (data)}));
            }
